...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """YOUR CODE"""
        input = [1] + inActs
        weighted_input = self.sigmoid(self.getWeightedSum(input))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        value = self.sigmoidActivation(inActs)
        sig_deriv = value * (1 - value)
        return sig_deriv
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in range(len(self.weights)):
            mod = alpha * delta * input[weight]
            self.weights[weight] = self.weights[weight] + mod
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        """YOUR CODE"""
        vals_out = []
        vals_out.append(inActs)
        currentList = vals_out[0]
        for i in self.layers:
            list = []
            for j in i:
                list.append(j.sigmoidActivation(currentList))
            vals_out.append(list)
            currentList = list
        return vals_out
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for input, output in examplesTest:
        out_list = nnet.feedForward(input)[-1]
        equal = True
        for i in range(len(out_list)):
            if round(out_list[i]) != output[i]:
                equal = False
        if equal:
            testCorrect += 1
        else:
